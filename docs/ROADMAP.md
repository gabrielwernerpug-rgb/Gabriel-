...
## 🟡 P4 — Aplicar as fotos das plantas no app

As **5** espécies que usavam hotlink direto ao Unsplash já têm foto gerada e
otimizada em `assets/plants/`. Falta aplicá-las no projeto Lovable — hoje elas
existem só aqui no repositório, e `plants.ts` continua com os hotlinks.

Para cada planta: salvar o arquivo em `src/assets/`, importar no topo de
`plants.ts` (mesmo padrão de `plant-orchid.jpg`) e apontar `img` e `thumb` para
o import.

| Chave | Arquivo |
|---|---|
| `samambaia` | `plant-fern.jpg` |
| `monstera` | `plant-monstera.jpg` |
| `cacto` | `plant-cactus.jpg` |
| `girassol` | `plant-sunflower.jpg` |
| `rosa` | `plant-rose.jpg` |
